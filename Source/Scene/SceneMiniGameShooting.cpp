...
void SceneGameShooting::Initialize()
{
	...
	stage->SetPosition(DirectX::XMFLOAT3(-22.8f, 1.0f, -26.0f));
	goalPos = { -22.0f, 1.0f, -70.5f };

	game_timer = 15;

	//プレイヤー初期化
	player = std::make_unique<PlayerShooting>();
	...
	for (int i = 0; i < 20; i++)
	{
		...
		std::mt19937 gen(rd());
		std::uniform_real_distribution<float>dist(-26.8f, -18.8f);
		std::uniform_real_distribution<float>dist2(1.5f, 5.0f);
		std::uniform_real_distribution<float>dist3(-21.0f, -16.0f);
...
// 終了化
void SceneGameShooting::Finalize()
{
...
// GUI描画
void SceneGameShooting::DrawGUI()
{
	//プレーヤーデバッグ処理
	//player->DrawDebugGUI();
	...
	//// 現在値取得
	//DirectX::XMFLOAT3 scale = stage->GetScale();
	//DirectX::XMFLOAT3 position = stage->GetPosition();
...
//UI
void SceneGameShooting::RenderUI(RenderContext rc)
{
	// スクリーンサイズ取得
	float screenWidth = Graphics::Instance().GetScreenWidth();
	float screenHeight = Graphics::Instance().GetScreenHeight();
